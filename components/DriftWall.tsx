...
/**
 * DriftWall（适配版）—— 3D 透视漂移墙。
 * 源自 ReactBits（reactbits.dev）的 DriftWall，为纯 rAF 实现、无第三方依赖。
 * 已按本站用途调整：tile 点击触发 onSelect(idx)（打开荣誉 lightbox），而非外链跳转。
 *
 * ⚠ 许可提醒：ReactBits 组件为开源第三方代码，正式发布前请确认其许可条款
 *   与归属声明（见 RUNNING.md 交接清单）。
 */
import {
  CSSProperties,
  useCallback,
...
export interface DriftItem {
  image: string;
  title?: string;
  /** 对应原始荣誉在父级数组里的下标，用于 lightbox 定位 */
  index: number;
}
...
const DriftWall = ({
  ...
  const [containerHeight, setContainerHeight] = useState<number>(height ?? 600);
  ...
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(([entry]) => {
      setContainerHeight(entry.contentRect.height || height || 600);
    });
