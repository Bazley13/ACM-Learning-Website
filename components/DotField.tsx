...
/**
 * DotField —— 全屏点阵背景（纯 rAF，无第三方依赖）。
 * 源自 ReactBits（reactbits.dev）。用于首页首屏的科技感动态背景。
 * ⚠ 第三方开源组件，正式发布前请确认许可条款（见 RUNNING.md 交接清单）。
 */
...
interface DotFieldProps {
  ...
  gradientTo?: string;
  glowColor?: string;
  text?: string; // 非空时，点阵只在文字笔画内生成（用文字蒙版）
  fontSize?: number; // 文字蒙版的字号
  [key: string]: unknown;
}
...
const DotField = memo(
  ({
    ...
    sparkle = false,
    waveAmplitude = 0,
    gradientFrom = "rgba(90, 160, 255, 0.5)",
    gradientTo = "rgba(120, 80, 255, 0.25)",
    glowColor = "#2a3b66",
    text,
    fontSize,
    ...rest
    ...
    propsRef.current = {
      ...
      gradientFrom, gradientTo, text, fontSize,
    };
    ...
    useEffect(() => {
      ...
      function buildDots(w: number, h: number) {
        ...
        // 文字蒙版：非空时只让落在文字笔画内的网格点生成点阵（如“ACM”）。
        let mask: Uint8ClampedArray | null = null;
        const text = p.text as string | undefined;
        if (text) {
          const fontSize = (p.fontSize as number) ?? Math.max(120, Math.min(h, 320) * 0.5);
          ...
          if (octx) {
            ...
            octx.font = `900 ${fontSize}px "JetBrains Mono", ui-monospace, Consolas, monospace`;
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            if (mask) {
              const px = Math.min(w - 1, Math.floor(ax));
              const py = Math.min(h - 1, Math.floor(ay));
              if (mask[(py * w + px) * 4 + 3] < 60) continue;
            }
      ...
      function tick() {
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (distSq < crSq && eng > 0.01) {
            ...
            if (isBulge) {
              const t2 = 1 - dist / cr;
              const push = t2 * t2 * (p.bulgeStrength as number) * eng;
    ...
    useEffect(() => {
      rebuildRef.current?.();
    }, [dotRadius, dotSpacing, text, fontSize]);
